...
*smacofRandomConfiguration()* which is mainly useful for theoretical studies of convergence and local minima. 
...
.. Note that $B$ is positive semi-definite. The term 
...
we majorize the quadratic to arrive at a simpler unweighted quadratic in $C$. 
In the Guttman initial configuration we maximize
the middle term using the alternative quartic constraint $\text{tr}\ (X'X)^2=\text{tr}\ C^2=1$. The stationary equations are $BX=X(X'X)$ and thus the maximum is attained for $\smash{X=K\Lambda^\frac12}$, where $\Lambda$
